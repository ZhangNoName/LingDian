...
if test -z "$upstream"; then
  /usr/bin/git -c http.version=HTTP/1.1 push -u origin "HEAD:refs/heads/$branch"
elif test "$upstream" = "origin/$branch"; then
  /usr/bin/git -c http.version=HTTP/1.1 push origin "HEAD:refs/heads/$branch"
else
...
Do not use `--no-verify`, override `core.hooksPath`, synthesize hook allow files, or route around a local security/DLP hook. If a hook, GitHub, authentication, network, or non-fast-forward check rejects the push, stop and report the exact blocker without switching transport or retrying through another upload mechanism.
