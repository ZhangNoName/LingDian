...
应用回滚不会自动反向执行数据库迁移。脚本会比较当前与目标 release 的完整 Prisma
迁移树（相对路径和文件内容），并拒绝缺失、空、不可读、结构异常或包含软链接的历史。
只有两棵迁移树完全一致才允许只回滚应用；不一致会在激活目标前拒绝，当前健康服务保持
运行。需要改变数据库状态时，应 roll-forward，或按最近的迁移前备份执行经演练的恢复。
恢复前必须先在隔离 MySQL 中导入 SQL、核对行数和关键业务数据，不能把第一次恢复尝试
放在生产库。
...
- 构建或迁移前备份失败：现有应用不切换；迁移尚未开始。
- 迁移前先停止 API，阻断旧代码继续写库；迁移或初始化失败时核心服务保持停止，不会自动重启旧 API。只能 roll-forward，或恢复已验证的迁移前备份后再启动匹配 release。
- 新容器或 TLS 健康门禁失败：仅当上一 release 与目标 release 的完整 Prisma 迁移树完全一致时自动恢复上一应用；否则停止核心服务并要求人工处置。
- 手工应用回滚若迁移树不完全一致，会在任何激活/停服动作前拒绝并保持当前服务运行。
