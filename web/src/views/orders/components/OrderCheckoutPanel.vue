<template>
  <Card
    class="flex h-full min-h-0 flex-col overflow-hidden rounded-lg border-border/80"
  >
    <CardHeader
      ...
      <div class="min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
        <Card class="rounded-lg border-border/80">
          <CardHeader class="pb-3">
            ...
            <CardDescription
              >输入会员号自动识别折扣，也可以一键填充示例会员。</CardDescription
            >
          </CardHeader>
          <CardContent class="grid gap-4">
            <div class="flex gap-2">
              <Input
                ...
                class="rounded-md"
                @update:model-value="
                  $emit('update:memberPhone', String($event))
                "
              />
              <Button
                variant="outline"
                class="rounded-md"
                @click="$emit('fillDemoMember')"
                >示例会员</Button
              >
            </div>
            <div
              class="flex items-center justify-between gap-3 rounded-md bg-muted/40 p-3"
            >
              <div>
                ...
                <p class="mt-1 text-xs text-muted-foreground">
                  {{
                    memberProfile
                      ? `${memberProfile.level} · 会员号 ${memberPhone}`
                      : "未匹配到会员，当前按原价结算。"
                  }}
                </p>
              </div>
        ...
        <Card class="rounded-lg border-border/80">
          <CardHeader class="pb-3">
            ...
            <CardDescription
              >在结算前选择可用优惠券，系统会自动计算优惠金额。</CardDescription
            >
          </CardHeader>
          <CardContent class="grid gap-4">
            <Select
              :model-value="selectedCouponCode"
              @update:model-value="
                $emit('update:selectedCouponCode', String($event))
              "
            >
            ...
            <div class="grid gap-2 rounded-md bg-muted/40 p-3 text-sm">
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="text-emerald-600"
                  >- ¥ {{ couponDiscountAmount.toFixed(2) }}</span
                >
              </div>
            </div>
          </CardContent>
                  ...
                  <TableCell>
                    <div class="flex items-center gap-2">
                      <Button
                        ...
                        @click="$emit('decreaseItem', item.id)"
                      >
                        -
                      </Button>
                      <span class="min-w-5 text-center text-sm">{{
                        item.quantity
                      }}</span>
                      <Button
                        variant="outline"
                        size="icon"
                        class="size-8 rounded-md"
                        @click="$emit('increaseItem', item.id)"
                      >
                        +
                      </Button>
                    </div>
        ...
        <Card class="rounded-lg border-border/80">
          <CardContent class="grid gap-3 p-5">
            ...
            <div class="flex items-center justify-between text-sm">
              ...
              <span class="text-emerald-600"
                >- ¥ {{ memberDiscountAmount.toFixed(2) }}</span
              >
            </div>
            <div class="flex items-center justify-between text-sm">
